    ...
    #Otherise, create a list of all datapoints for each metric from their log file
        ...
        for metric in metric_list:
            if path.exists(metric['path']):
                ...
                metric_row = {"name":metric['name'],  "date":timestamp, "error": 'error accessing log file. Check path in clientparams.py'}
                metrics.append(metric_row)
        return metrics
...
test_output(rebuild_metric_db=False)
#metric_list = get_metric_list(param_path)
#metric_data = get_metrics(metric_list, rebuild_metric_db=True)
#print(metric_data)
#print(metric_data)
